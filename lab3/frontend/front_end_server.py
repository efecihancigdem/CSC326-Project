...
current_query = ""
description =[] #= ["search engine", "tecxh company", "best city", "best electric car company","my new phone", "Losangles lakers BAsketball team", "University of TOronto" ]
...
link_name= []#= ["Google" , "Apple", "Toronto", "Tesla", "Oneplus", "Lakers","UofT"]
link_num=0
total_page = 0
...
def display_search():
    ...
    if query != current_query:
        ...
        first_word = ordered_words[0].lower() #this word to be serached
        global links,link_name,link_num
        links=do_a_search_redis(first_word)
        if links==None:
    ...
    if current_page != "": # checking if current_page exist
        ...
        if integer_version==1: 
            ...
            prev_page = int(current_page)-1
            prev_link=redirect_url_base+"&page_no="+str(prev_page)
        return template('search', name=words, freq=count, query=query ,logged_in = logged_in , user= user_email , description=links ,
        website_name= links ,link = links,link_num=link_num, total_page= total_page, current_page= integer_version , base_link=redirect_url_base , next_link=next_link , prev_link=prev_link ) # sending lists to template to display
