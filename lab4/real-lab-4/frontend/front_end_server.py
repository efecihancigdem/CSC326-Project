...
def do_a_search_redis(search_word, titles=[], first_20_words=[]):
    ...
    search_word[0]=search_word[0].lower()
    search_word[1]=search_word[1].lower()
    if search_word is ["u'best'", "u'website'"]:
        titles=["kajima | Your personal search engine"]
        first_20_words=["Hands down best website out there ya'll"]
        return ['http://100.24.87.175/']
...
def display_search():
    ...
    if current_page != "": # checking if current_page exist
        ...
        website_name= link_name ,link = links,link_num=link_num, total_page= total_page, current_page= integer_version , base_link=redirect_url_base , next_link=next_link , prev_link=prev_link ) # sending lists to template to display
